...
export default function ClientGigDetailScreen({ route, navigation }) {
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      <View style={styles.clientCard}>
        <View style={styles.clientAvatar}>
          {gig.clientInfo?.avatarUrl ? (<Image source={{ uri: gig.clientInfo.avatarUrl }} style={styles.avatarImage} />) : (<Text style={styles.avatarInitial}>{(gig.clientInfo?.fullName || 'Y').charAt(0).toUpperCase()}</Text>)}
        </View>
        <View style={styles.clientInfo}>
          <Text style={styles.clientName}>{gig.clientInfo?.fullName}</Text>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.detailRow}>
          <Text style={styles.detailLabel}>Proposals Received:</Text>
          <Text style={styles.proposalsCount}>{proposalsCount}</Text>
        </View>
      </View>
      ...
      <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()}><Text style={styles.backButtonText}>← Back to My Gigs</Text></TouchableOpacity>
    </ScrollView>
  );
}
